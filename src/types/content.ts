...
export interface Question {
  /** Stable sequential id (1..310). Used as the React `key` and to link a card to its deep dive. */
  n: number;
   ...
   * safe later in `QuestionCard` — see the comment there before copying this
   * pattern anywhere that touches untrusted data.
   */
  a: string;
  /** Answer, Russian translation. Same trusted-HTML rules as `a`. */
  aru: string;
  /**
   * Optional "deep dive" lesson body (trusted HTML), merged onto the question
   * at load time from `src/data/deepDives.ts` — see `src/data/index.ts`.
   * Only ~110 of the 310 questions currently have one.
   */
  d?: string;
}
